...
function printRequest(req){
	...
	//CUSTOM START
	let domainPattern = /^https?:\/\/([a-zA-Z0-9-]+\.)*vimeo\.com\//;
	if (!req.url.match(domainPattern)) {
    	// req.abort('aborted');
    	return;
	}
	//CUSTOM END
...
async function printLinks(rootNode, page){
	...
	for(let l of links){
		t = await safeJsonValue(await l.getProperty("href"));
		req = { type: "link", url: t};
		printRequest(req);
	}
	var metas = await el.$$('meta[http-equiv="refresh"]');
	for(let m of metas){
		t = await safeJsonValue(await m.getProperty("content"));
		t = t.split(";");
...
async function getFormAsRequest(frm, page){
	...
	formObj.method = await safeJsonValue(await frm.getProperty("method"));
	...
	formObj.url = await safeJsonValue(await frm.getProperty("action"));
	...
	for(let input of inputs){
		let name = await safeJsonValue(await frm.getProperty("name"));
		if(!name) continue;
		let value = await safeJsonValue(await frm.getProperty("value"));
		let tagName = await safeJsonValue(await frm.getProperty("tagName"));
		let type = await safeJsonValue(await frm.getProperty("type"));
		...
		if(tagName == "INPUT" && type != null){
			...
			switch(type.toLowerCase()){
				...
				case "checkbox":
				case "radio":
					let checked = await safeJsonValue(await frm.getProperty("checked"));
					if(checked)
	...
	for(var a = 0; a < args.opts.length; a++){
		switch(args.opts[a][0]){
			...
			case "i":
				...
				break;
			case "K":
				options.preventElementRemoval = false;
				break;
			case "R":
