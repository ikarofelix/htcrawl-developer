...
function initProbe(options, inputValues){
	console.log("Initializing Probe on the page...");
	...
	*/
		...
		var pdh = function(e){
			...
			*/
			...
			if (el.form) { 
				...
				// let actionURL = el.form.getAttribute('action') || window.location.href;
				// let enctype = el.form.getAttribute('enctype');
				// let headers = {}; 
                // if (enctype === 'multipart/form-data') {
                //     // Let the browser set the content type with the appropriate boundary parameter for multipart data
	...
	Probe.prototype.getElementSelector = function(element) {
		...
		const className = element.className && typeof element.className === 'string' ? element.className.trim().split(/\s+/).join('.') : null;
		if (className) {
			try{
				...
				console.error(`Error processing element's class selector:`, error);
			}
		}
		...
		if (element !== document.documentElement && name !== "body" && element.parentNode) {
			selector = `${this.getElementSelector(element.parentNode)} > ${selector}`;
		}
	...
	// 	formObj.method = form.getAttribute("method") || "GET";
	// 	formObj.method = formObj.method.toUpperCase();
	
	// 	formObj.url = form.getAttribute("action") || document.location.href;
	// 	formObj.data = [];
	...
	// 		if(inputs[a].tagName == "INPUT" && inputs[a].type){
	// 			switch(inputs[a].type.toLowerCase()){
	// 				case "button":
	// 				case "submit":
	// 					break;
	// 				case "checkbox":
	// 				case "radio":
	// 					if(inputs[a].checked) formObj.data.push(par);
	// 					break;
	// 				default:
	// 					formObj.data.push(par);
	// 			}
	...
	// 	if(formObj.method == "GET"){
	// 		var url = this.replaceUrlQuery(formObj.url, formObj.data);
	// 		customReq = new this.Request("form", "GET", url);
	// 	} else {
	// 		customReq = new this.Request("form", "POST", formObj.url, formObj.data, this.getTrigger());
	// 	}
	
	// 	return {
	// 		p: {
	// 			// This part is a stub. In a real scenario, you'd get this from the actual HTTPRequest instance.
	// 			_requestId: 'XXXX.XX',
	// 			_failureText: null,
	// 			_response: null,
	// 			_fromMemoryCache: false,
	// 			_redirectChain: [],
	// 			_interceptionId: 'interception-job-XX.X'
	// 		},
	// 		h: customReq
	// 	};
	// };
